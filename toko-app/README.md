...
Buka `http://localhost:3000` untuk etalase, dan `http://localhost:3000/admin-x9k2f` untuk dashboard admin
...
Folder `src/app/admin-x9k2f/` adalah path rahasia dashboard admin. **Sebelum deploy ke produksi**:

1. Rename folder tersebut menjadi string acak panjang milik Anda sendiri, contoh:
   ```bash
   mv src/app/admin-x9k2f src/app/kelola-Xy82fLpQz9
   ```
2. Jangan pernah link-kan path ini dari halaman publik manapun (navbar, footer, sitemap).
3. Password login (`ADMIN_PASSWORD`) adalah lapisan kedua — jadi walau seseorang menebak path-nya, mereka masih
   butuh password untuk masuk dashboard.
4. Disarankan tambahkan juga proteksi di level hosting (mis. Vercel Password Protection / IP allowlist) sebagai
